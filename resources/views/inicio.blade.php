...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-banner:active { transform: translateY(0); }
        .btn-banner.descuento {
            background: #00a650;
            color: white;
        }
        .btn-banner.descuento:hover { background: #008f45; }
        .seccion-destacados {
            max-width: 1200px;
            margin: 0 auto 30px;
            padding: 0;
        ...
        .titulo-seccion.descuento { color: #00a650; }
        .titulo-seccion.descuento::after { background: #00a650; }
        .productos-grid {
            display: grid;
        ...
        .precio-container { margin-bottom: 8px; }
        .precio-original {
        ...
        .ver-detalle a:hover { text-decoration: underline; }
        .btn {
            display: inline-block;
            padding: 12px 25px;
            background: #007bff;
        ...
        .btn-descuento { background: #00a650; }
        .btn-descuento:hover { background: #008f45; }
        .corazon-favorito {
            position: absolute;
            top: 10px;
            right: 10px;
        ...
        .corazon-favorito:active { transform: scale(0.95); }
        .corazon-favorito.activo {
            color: #ff4757;
            background: rgba(255,71,87,0.1);
            border-color: #ff4757;
        }
        .corazon-favorito.inactivo { color: rgba(0,0,0,0.25); }
        .toast-single {
            position: fixed;
            top: 20px;
            right: 20px;
        ...
        .pagination li:first-child a::before { content: "←"; font-size: 18px; }
        .pagination li:last-child a::before { content: "→"; font-size: 18px; }
        ...
        @media (max-width: 768px) {
            ...
            .barra-busqueda-principal input { font-size: 14px; padding: 10px 15px; }
            .barra-busqueda-principal button { padding: 10px 18px; font-size: 14px; }
            .nav-links { gap: 12px; padding: 0 10px; }
            .nav-links li a,
            .nav-links li button { font-size: 13px; }
            .badge-variacion { font-size: 10px; padding: 3px 6px; }
        }
        @media (max-width: 480px) {
            ...
            .banner-inicio { padding: 25px 15px; }
            .nav-links {
                flex-direction: column;
                gap: 8px;
                align-items: center;
...
<body>
    ...
    <nav class="navbar">
        <div class="nav-container">
            <ul class="nav-links">
                ...
                <li>
                    @auth
                        <a href="{{ route('favoritos.index') }}" style="color: #495057;">❤️ Mis Favoritos</a>
                    @else
                        <a href="{{ route('login') }}" style="color: #495057;">❤️ Mis Favoritos</a>
                    @endauth
                </li>
                @auth
        ...
        <div class="barra-busqueda-principal">
            <form action="{{ route('busqueda.resultados') }}" method="GET">
                <input type="text" name="q" placeholder="Buscar productos..." value="{{ request('q') }}" autocomplete="off">
    ...
    <div class="seccion-destacados">
        <h2 class="titulo-seccion descuento">🔥 Productos con Descuento</h2>
                ...
                <div class="producto-card" onclick="window.location.href='{{ $url }}'">
                    <div class="producto-imagen-container">
                        <button class="corazon-favorito {{ $esFavorito ? 'activo' : 'inactivo' }}" 
                                ...
                                title="{{ $esFavorito ? 'Quitar' : 'Agregar' }}">
                            {{ $esFavorito ? '❤️' : '🤍' }}
                        </button>
                        @if($tieneDescuento)
                            <div class="badge-descuento">-{{ $porcentajeDescuento }}% OFF</div>
                        @elseif($estaBajoStock)
                            <div class="badge-stock-bajo">¡Últimas!</div>
                        @endif
                        @if($esVariacion && !empty($atributosTexto))
                            <div class="badge-variacion" title="{{ $atributosTexto }}">{{ $atributosTexto }}</div>
                        @endif
                        @if(count($imagenes) > 0)
                            <img src="{{ $imagenes[0] }}" alt="{{ $nombreProducto }}" class="producto-imagen">
                        @else
                            <div class="no-imagen"><span>🛒 Sin imagen</span></div>
                        @endif
                    </div>
                    <div class="producto-info">
                        ...
                        <div class="stock-info {{ $stock > 10 ? 'stock-bueno' : ($stock > 0 ? 'stock-bajo' : 'sin-stock') }}">
                            @if($stock > 10) ✅ En stock ({{ $stock }} disponibles)
                            @elseif($stock > 0) ⚠️ Solo {{ $stock }} unidades
                            @else ❌ Sin stock
                            @endif
                        </div>
                        <p style="font-size: 13px; color: #666;">{{ $marca }}</p>
                        <div class="ver-detalle"><a href="{{ $url }}" onclick="event.stopPropagation();">Ver más</a></div>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <div class="seccion-destacados">
                ...
                @foreach($productosDestacados as $item)
                    ...
                    <div class="producto-card" onclick="window.location.href='{{ $url }}'">
                        <div class="producto-imagen-container">
                            ...
                            @if($tieneDescuento)
                                <div class="badge-descuento">-{{ $porcentajeDescuento }}%</div>
                            @elseif($estaBajoStock)
                                <div class="badge-stock-bajo">¡Últimas!</div>
                            @endif
                            @if($esVariacion && !empty($atributosTexto))
            ...
            @foreach($productosRecomendados as $item)
                ...
                <div class="producto-card" onclick="window.location.href='{{ $url }}'">
                    ...
                    <div class="producto-info">
                        ...
                        <div class="stock-info {{ $stock > 10 ? 'stock-bueno' : ($stock > 0 ? 'stock-bajo' : 'sin-stock') }}">
                            @if($stock > 0) ✅ Disponible ({{ $stock }} unidades)
                            @else ❌ Sin stock
                            @endif
                        </div>
                        <div class="ver-detalle"><a href="{{ $url }}" onclick="event.stopPropagation();">Ver más</a></div>
        ...
        <div class="paginacion">{{ $todosLosProductos->links() }}</div>
        <div style="text-align: center;"><a href="{{ route('busqueda.resultados') }}" class="btn">Ver Todos los Productos</a></div>
        @endif
    </div>

    <script>
        let singleToast = null, singleToastTimeout = null;
        function toggleFavorito(button, productoId, variacionId = null) {
            ...
            removeSingleToast();
            let url = variacionId ? `/favoritos/toggle-variacion/${variacionId}` : `/favoritos/toggle-producto/${productoId}`;
            fetch(url, {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    if (data.action === 'added') {
                        button.classList.replace('inactivo', 'activo');
                        button.innerHTML = '❤️';
                        showSingleNotification('✅ ' + (data.tipo === 'variacion' ? 'Variación' : 'Producto') + ' agregado a favoritos', 3000);
                    } else {
                        button.classList.replace('activo', 'inactivo');
                        button.innerHTML = '🤍';
                        showSingleNotification('❌ ' + (data.tipo === 'variacion' ? 'Variación' : 'Producto') + ' eliminado de favoritos', 3000);
                    }
                } else showSingleNotification('❌ Error al gestionar favoritos', 3000);
            })
            .catch(() => showSingleNotification('❌ Error de conexión', 3000))
            .finally(() => {
                setTimeout(() => {
        ...
        function removeSingleToast() {
            if (singleToast) {
                ...
                setTimeout(() => { if (singleToast?.parentNode) singleToast.remove(); singleToast = null; }, 300);
            }
            if (singleToastTimeout) clearTimeout(singleToastTimeout);
            document.querySelectorAll('.toast-single').forEach(t => t.remove());
        }
        function showSingleNotification(message, duration = 3000) {
            ...
            toast.innerHTML = `<span style="font-size:20px;">${emoji}</span><span>${message.replace('✅','').replace('❌','').trim()}</span>`;
            document.body.appendChild(toast);
            singleToast = toast;
            setTimeout(() => toast.classList.add('show'), 10);
            singleToastTimeout = setTimeout(() => {
                toast.classList.remove('show');
                setTimeout(() => { if (toast.parentNode) toast.remove(); singleToast = null; }, 400);
            }, duration);
        }
        document.addEventListener('DOMContentLoaded', function() {
            if (window.innerWidth > 768) document.querySelector('.barra-busqueda-principal input')?.focus();
            document.querySelectorAll('.producto-card button, .producto-card a').forEach(b => b.addEventListener('click', e => e.stopPropagation()));
        });
    </script>
</body>
</html>