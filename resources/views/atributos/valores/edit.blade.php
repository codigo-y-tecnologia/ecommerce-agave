...
// Inicializar: guardar el slug actual como último generado
document.addEventListener('DOMContentLoaded', function() {
    const slugInput = document.getElementById('vSlug');
    const valorInput = document.getElementById('vValor');
    
    // Si el slug actual es igual al que se generaría automáticamente, no está editado
    const slugAutoGenerado = generarSlug(valorInput.value.trim());
    if (slugInput.value === slugAutoGenerado) {
        slugEditadoManualmente = false;
        ultimoSlugGenerado = slugInput.value;
    } else {
...
document.getElementById('vValor').addEventListener('input', function() {
    ...
    // 2. El slug no ha sido editado manualmente, O
    // 3. El slug actual es igual al último generado automáticamente
    if (!slugEditadoManualmente || slugInput.value === '' || slugInput.value === ultimoSlugGenerado) {
        if (valor) {
...
document.getElementById('vSlug').addEventListener('focus', function() {
    ...
    if (this.value === valorSlug || this.value === '') {
        slugEditadoManualmente = false;
    } else {
        slugEditadoManualmente = true;
    }
...
document.getElementById('vSlug').addEventListener('input', function() {
    ...
    // Si el usuario edita manualmente, marcar como editado
